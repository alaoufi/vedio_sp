...
data class LibraryQuery(
    ...
    fun toSupportQuery(): SupportSQLiteQuery {
        ...
        val orderBy = if (continueOnly) {
            ...
            // Rank by the most recent activity — the later of "last watched" and
            // "added" — so a recently-watched clip AND a freshly-downloaded one both
            // surface at the top; a never-watched new download no longer sinks.
            SortOrder.LAST_PLAYED_DESC ->
                "ORDER BY MAX(COALESCE(last_played_date, 0), created_date) DESC, created_date DESC"
